...
describe 'Pay an order with Paymill' do
  context 'with paymill keys' do
    before(:each) do
      ...
      paymill_payment_method = Spree::Gateway::PaymillCreditCard.create!(
        name: 'Credit Card',
        preferences: {
          public_key: '121781582632ed623ceeb1ed354e423c', #'['TEST_PAYMILL_PUBLIC_KEY'],
          private_key: 'ab1f680d9c41f34a3dc5cd86479c7b18' # ENV['TEST_PAYMILL_PRIVATE_KEY']
        }
      )
  ...
  context 'without paymill key' do
    scenario "a paymill bridge error occured while paying an order with a creditcard ", js: true do
      ...
      allow(order).to receive_messages(:available_payment_methods => [ paymill_payment_method, create(:check_payment_method) ])
      order.user = create(:user)
      order.update!
      allow_any_instance_of(Spree::CheckoutController).to receive_messages(:current_order => order)
      allow_any_instance_of(Spree::CheckoutController).to receive_messages(:try_spree_current_user => order.user)
